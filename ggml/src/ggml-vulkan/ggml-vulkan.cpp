...
#undef CREATE_MM
    ...
    const bool allow_gcn_subgroups =
        ...
        (device->subgroup_require_full_support ||
         (device->subgroup_min_size == device->subgroup_max_size && device->subgroup_min_size == 64));

    const bool use_subgroups = device->subgroup_arithmetic &&
                                (device->architecture != vk_device_architecture::AMD_GCN || allow_gcn_subgroups);
    ...
    for (uint32_t w = 0; w < DMMV_WG_SIZE_COUNT; ++w) {
        ...
        const bool subgroup_reduction = use_subgroups && w == DMMV_WG_SIZE_SUBGROUP;
        const bool subgroup_reduction16 = use_subgroups16 && w == DMMV_WG_SIZE_SUBGROUP;

        const shader_reduction_mode reduc = subgroup_reduction ? SHADER_REDUCTION_MODE_SUBGROUP :
                                            (use_subgroups && w == DMMV_WG_SIZE_LARGE) ? SHADER_REDUCTION_MODE_HYBRID :
                                            SHADER_REDUCTION_MODE_SHMEM;

        const shader_reduction_mode reduc16 = subgroup_reduction16 ? SHADER_REDUCTION_MODE_SUBGROUP :
                                              (use_subgroups16 && w == DMMV_WG_SIZE_LARGE) ? SHADER_REDUCTION_MODE_HYBRID :
                                              SHADER_REDUCTION_MODE_SHMEM;

        const bool require_full_subgroups = subgroup_reduction ? true : use_subgroups;
        const bool require_full_subgroups16 = subgroup_reduction16 ? true : use_subgroups16;

        const uint32_t required_subgroup_size = subgroup_reduction && allow_gcn_subgroups ? device->subgroup_size : force_subgroup_size;
        const uint32_t required_subgroup_size16 = subgroup_reduction16 && allow_gcn_subgroups ? device->subgroup_size : force_subgroup_size16;

        for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[w][GGML_TYPE_F32 ][i], "mul_mat_vec_f32_f32_f32",  arr_dmmv_f32_f32_f32_len[reduc],  arr_dmmv_f32_f32_f32_data[reduc],  "main", 3, sizeof(vk_mat_vec_push_constants), {2, 1, 1}, {wg_size_subgroup, 2, i+1}, 1, false, require_full_subgroups, required_subgroup_size);
            ...
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[w][GGML_TYPE_BF16][i], "mul_mat_vec_bf16_f32_f32", arr_dmmv_bf16_f32_f32_len[reduc], arr_dmmv_bf16_f32_f32_data[reduc], "main", 3, sizeof(vk_mat_vec_push_constants), {2, 1, 1}, {wg_size_subgroup, 2, i+1}, 1, false, require_full_subgroups, required_subgroup_size);
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[w][GGML_TYPE_Q4_0][i], "mul_mat_vec_q4_0_f32_f32", arr_dmmv_q4_0_f32_f32_len[reduc], arr_dmmv_q4_0_f32_f32_data[reduc], "main", 3, sizeof(vk_mat_vec_push_constants), {2*rm_stdq, 1, 1}, {wg_size_subgroup, 2*rm_stdq, i+1}, 1, true, require_full_subgroups, required_subgroup_size);
